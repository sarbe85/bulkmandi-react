...
const schema = z.object({
  ...
  ifscCode: z.string().regex(/^[A-Z0-9]{11}$/, 'Invalid IFSC code'),
...
export const BankDocsStep = ({ data, onNext, onBack }: Props) => {
  ...
  const [documents, setDocuments] = useState<
    Map<DocumentType, { file: File; url: string }>
  >(new Map());
  ...
  const handleFileChange = (type: DocumentType, file: File | null) => {
    if (file) {
      ...
      newDocs.set(type, { file, url });
      setDocuments(newDocs);
    }
  };

  // ✅ MODIFIED: Now calls API and passes response to onNext
  const onSubmit = async (formData: FormData) => {
    try {
      ...
      for (const [type, { file, url }] of documents.entries()) {
      ...
      const response = await onboardingService.updateBankDetails(bankDocData);
                ...
                <label htmlFor={`file-${doc.type}`} className="cursor-pointer">
                  {documents.has(doc.type) ? (
                    <div className="flex items-center justify-center gap-2 text-green-600">
                      ...
                      <span>{documents.get(doc.type)?.file.name}</span>
                    </div>
                  ) : (
                    <>
