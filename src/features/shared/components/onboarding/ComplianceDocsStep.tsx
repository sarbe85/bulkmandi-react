...
export default function BankDetailsStep({ data, onNext, onBack }: Props) {
  ...
  const canSubmit = !!accountNumber && !!accountHolderName && !!ifsc && ifscValidated && uploadedDocs.has("CANCELLED_CHEQUE") && !isSaving;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="space-y-3">
            ...
            <div className="mt-1.5 flex gap-4">
              {payoutMethods.map((pm) => (
                <label key={pm} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" value={pm} {...register("payoutMethod")} className="h-3.5 w-3.5" disabled={isSubmitting || isSaving} />
        ...
        {payoutMethod === "UPI" && (
          <div className="mt-3">
            ...
            <Input {...register("upiDetails")} placeholder="example@upi" disabled={isSubmitting || isSaving} className="mt-1.5 h-9 text-sm" />
      ...
      <div className="flex items-center justify-between pt-3 border-t">
        ...
        <Button type="submit" disabled={!canSubmit || isSubmitting || isSaving} size="sm" className="h-9 text-sm">
